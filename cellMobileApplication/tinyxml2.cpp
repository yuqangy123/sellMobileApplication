...
#else
        int len = vsnprintf_s( 0, 0, 1, format, va );
        // Close out and re-start the va-args
        va_end( va );
        va_start( va, format );
        char* p = _buffer.PushArr( len ) - 1;
		vsnprintf_s(p, len + 1, 1, format, va);
#endif
    }
    va_end( va );
}
...
ValueMap getValueMapFromXmlElement(tinyxml2::XMLElement *element)
{
	ValueMap map;
	if (NULL == element)
	{
	...
	tinyxml2::XMLElement* nnd = element->FirstChildElement();
	if (nullptr != nnd)
	{
		...
		while (nnd)
		{
			const char* nndnode = nnd->Value();
			if (subMap.find(nndnode) == subMap.end())
			{
				ValueVector subVector;
				subMap[nndnode] = Value(subVector);
			}
			ValueVector& vtr = subMap[nndnode].asValueVector();
			vtr.push_back(Value(getValueMapFromXmlElement(nnd)));

			nnd = nnd->NextSiblingElement();
		}
		map[nodeName] = subMap;
	}
	else
	{
		map[nodeName] = element->GetText();
	}
